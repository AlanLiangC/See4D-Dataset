...
from see4d.utils.video_depth_est import Fast3R_Inferencer
...
def process_images(uploaded_files, video_file, state,
                   fast3r_egine, input_index, lit_module, device,
    ...
    #     profiling=True,
    # )
    output_dict, profiling_info = fast3r_egine.inference_sample(index=int(input_index))
...
def create_demo(checkpoint_dir, examples_dir, output_dir, device: torch.device, is_lightning_checkpoint=False):
    ...
    model, lit_module = fast3r_egine.model, fast3r_egine.lit_module,
    ...
    with gr.Blocks(title="Fast3R 3D Reconstruction Demo", ) as demo:
        ...
        gr.HTML(header_html)
        input_index = gr.Number("Smple index")
        with gr.Row():
        ...
        def process_images_wrapper(uploaded_files, video_file, state, image_resolution, input_index_value):
            ...
            generator = process_images(uploaded_files, video_file, state,
                                     fast3r_egine, input_index_value, lit_module, device,
                                     global_manager_req_queue, global_manager_resp_queue, 
                                     output_dir, examples_dir,
        ...
        submit_button.click(
            ...
            inputs=[gallery, video_input, state, image_resolution, input_index],
...
def main():
    ...
    parser.add_argument("--checkpoint_dir", type=str, default="pretrained_models/fast3r")
    parser.add_argument("--examples_dir", type=str, default="tools/viz/demo_examples")
    parser.add_argument("--output_dir", type=str, default="tools/viz/demo_outputs",
    ...
    demo.launch(share=True, server_port=7007)
