...
class BEN_Base(
    ...
    def inference_mask_rgb(self, image, refine_foreground=False):
        set_random_seed(9)
        foregrounds = []
        masks = []
        for batch in image:
            ...
            if refine_foreground == True:
                ...
                binary_img = mask.point(lambda p: 255 if p > 150 else 0)
                masks.append(binary_img)
...
def images2mp4(images, output_path, fps, rgb_value=(0, 255, 0)):
    pil_images_to_mp4(
        images, output_path, fps=fps, rgb_value=rgb_value
    )
